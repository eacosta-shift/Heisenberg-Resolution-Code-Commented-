...
def FisherInfo(a_i=0, phi=np.pi/2, G=1, t_s1=1, t_i1=1, t_s2=1):
    U = np.cosh(G)
    V = np.sinh(G)
    ...
    F=np.divide(num,dnum,out=np.zeros_like(num), where=num!=0)

    return F

def OptFisherInfo(a_i=0, G=1, t_s1=1, t_i1=1, t_s2=1):

    phi = OptimalPhase(a_i=a_i, G=G, t_s1=t_s1, t_i1=t_i1, t_s2=t_s2, dps=50)
